...
tremor_Data_arr = np.array([['http://www.tremorgames.com/playgame/5087/death-call.html','http://www.tremorgames.com/games/files/DeathCall_withAds.swf',63,'sf5tghy7uj'],
...
tremor_Data_arr2 = np.array([['http://www.tremorgames.com/playgame/5872/super-defence.html','http://www.tremorgames.com/games/files/ZI3_SuperShooterV4.3.swf',127,'fr5tgbhny67'],
...
i = 0
while i < 92:
    GameID = int(tremor_Data_arr[i,2])
    Key = (tremor_Data_arr[i,3])
    GameURL = (tremor_Data_arr[i,0])
    GameSWF = (tremor_Data_arr[i,1])
...
# Начало фарма ачивок
    ...
    # usually the current stat value isn't 0 but something like null or none, so we set it to 0
        ...
        print( 'Ачивка в оде получена\n' )
    print( 'Иг а закончена, по счёту она : ',i+1 )#;print ( '1');time.sleep( 1 );print ( '2');time.sleep( 1 );print ( '3');time.sleep( 1 );print ( '4');time.sleep( 1 );print ( '5')
    i = i + 1
i = 0
while (i < 74):
    GameID = int(tremor_Data_arr2[i,2])
    Key = (tremor_Data_arr2[i,3])
    GameURL = (tremor_Data_arr2[i,0])
    GameSWF = (tremor_Data_arr2[i,1])
    gameAchievements = GetGameAchievements( s )
#gameStats = GetGameStats( s, username )
